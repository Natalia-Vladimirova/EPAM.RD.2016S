...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result Load(string assemblyString, Input data)
        {
            ...
            var type = types
                .First(t => (t.GetCustomAttribute(typeof (DoSomethingAttribute)) != null) &&
                        (t.GetInterface("IDoSomething") != null));

            // Save instance to variable.
            IDoSomething doSomethingService = (IDoSomething)(Activator.CreateInstance(type)); 
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            // Find first type that has DoSomething attribute and don't implement IDoSomething.
            Type type = types
                .First(
                    t =>
                        (t.GetCustomAttribute(typeof (DoSomethingAttribute)) != null) &&
                        (t.GetInterface("IDoSomething") == null));
            var obj = Activator.CreateInstance(type);

            MethodInfo mi = type.GetMethod("DoSomething");
            Result result = (Result) mi.Invoke(obj, new object[] {data});
